...
describe('CSSStyleParser', () => {
  describe('parse', () => {
    ...
    it('ケバブケースをキャメルケースに変換する', () => {
      const css = 'margin-top: 10px; margin-bottom: 20px; background-color: #f0f8ff';
      const result = CSSStyleParser.parse(css);
    ...
    it('単位なしの数値はpx単位としてフォールバックする（警告付き）', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Unitless dimension "8" for property "padding"')
      );
      expect(consoleSpy).toHaveBeenCalledWith(
  ...
  describe('stringify', () => {
    it('スタイルオブジェクトをCSS文字列に変換できる', () => {
      ...
      expect(result).toBe('font-size: 14pt; margin-top: 8; background-color: #f0f8ff');
    });
  });

  describe('color handling', () => {
    test('should expand short hex colors to full hex', () => {
      const style = CSSStyleParser.parse('color: #333; background-color: #F0F; border-color: #abc');
    ...
    test('should leave full hex colors unchanged', () => {
      const style = CSSStyleParser.parse('color: #333333; background-color: #F0F0F0');
      expect(style).toEqual({
        color: '#333333',
    ...
    test('should not modify non-color properties with hash values', () => {
      const style = CSSStyleParser.parse('content: "#333"; font-family: "Arial"');
      expect(style).toEqual({
        content: '"#333"',
        fontFamily: '"Arial"'
      });
