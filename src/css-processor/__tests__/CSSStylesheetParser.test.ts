...
describe("CSSStylesheetParser", () => {
  ...
  describe("PowerPoint property support validation", () => {
    ...
    test("should generate warnings for unsupported properties", () => {
      ...
      expect(
        result.warnings.some((w) =>
          w.includes("justify-content is not supported"),
        ),
      ).toBe(true);
      expect(
        result.warnings.some((w) =>
          w.includes("display: grid is not supported"),
        ),
      ).toBe(true);
      expect(
        result.warnings.some((w) =>
          w.includes("position is not fully supported"),
        ),
      ).toBe(true);
    });
    ...
    test("should suggest alternatives for unsupported properties", () => {
      ...
      expect(
        result.warnings.some((w) =>
          w.includes("Use flex-direction for layout control"),
        ),
      ).toBe(true);
      expect(
        result.warnings.some((w) =>
          w.includes("Use padding for alignment control"),
        ),
      ).toBe(true);
    });
    ...
    test("should warn about properties with limited support", () => {
      ...
      expect(
        result.warnings.some((w) =>
          w.includes("box-shadow support is limited in PowerPoint"),
        ),
      ).toBe(true);
    });
    ...
    test("should warn about unknown properties", () => {
      ...
      expect(result.warnings.some((w) => w.includes("custom-property"))).toBe(
        true,
      );
      expect(
        result.warnings.some((w) => w.includes("support is unknown")),
      ).toBe(true);
    });
  });
  ...
  describe("selector support validation", () => {
    test("should only support class selectors", () => {
      ...
      // 非対応セレクタの警告
      expect(result.warnings.some((w) => w.includes("#id-selector"))).toBe(
        true,
      );
      expect(result.warnings.some((w) => w.includes("element"))).toBe(true);
      expect(result.warnings.some((w) => w.includes(".parent .child"))).toBe(
        true,
      );
    });
  ...
  describe("CSS parsing error handling", () => {
    test("should handle malformed CSS gracefully", () => {
      ...
      expect(result.warnings.some((w) => w.includes("CSS parsing error"))).toBe(
        true,
      );
    });
  });
  ...
  describe("utility methods", () => {
    ...
    test("getPropertySupport should return property support info", () => {
      ...
      const justifySupport =
        CSSStylesheetParser.getPropertySupport("justify-content");
      ...
      const unknownSupport =
        CSSStylesheetParser.getPropertySupport("unknown-property");
  ...
  describe("CSS variables (custom properties) support", () => {
    test("should fail when CSS variables are used (RED phase)", () => {
      ...
      expect(
        result.warnings.some(
          (warning) =>
            warning.includes("var(") ||
            warning.includes("CSS variable") ||
            warning.includes("custom property"),
        ),
      ).toBe(true);
    });
