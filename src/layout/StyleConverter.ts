  ...
  static convertDimensionUnit(
    ...
    if (typeof value === "string") {
      const supportedUnits =
        /^(auto|inherit|initial|unset|\d*\.?\d+(px|%|vw|vh|em|rem))$/i;
      if (!supportedUnits.test(value.trim())) {
        console.warn(
          `⚠️  Unsupported unit for ${propertyName}: "${value}". Supported units: px, %, vw, vh, em, rem, auto`,
        );
      }
    }
