...
function layoutNodeImpl(
  node,
  parentMaxWidth,
  /*css_direction_t*/ parentDirection,
) {
  ...
  var /*(c)!css_flex_direction_t*/ /*(java)!int*/ crossAxis =
  ...
  var /*float*/ paddingAndBorderAxisResolvedRow = getPaddingAndBorderAxis(
      node,
      resolvedRowAxis,
    );

  if (isMeasureDefined(node)) {
    var /*bool*/ isResolvedRowDimDefined = !isUndefined(
  ...
  var /*float*/ leadingPaddingAndBorderMain = getLeadingPaddingAndBorder(
      node,
      mainAxis,
    );
  var /*float*/ leadingPaddingAndBorderCross = getLeadingPaddingAndBorder(
      node,
      crossAxis,
    );
  var /*float*/ paddingAndBorderAxisMain = getPaddingAndBorderAxis(
      node,
      mainAxis,
    );
  var /*float*/ paddingAndBorderAxisCross = getPaddingAndBorderAxis(
      node,
      crossAxis,
    );
  ...
  while (endLine < childCount) {
    ...
    if (flexibleChildrenCount !== 0) {
      ...
      while (currentFlexChild !== null) {
        ...
        if (isDimDefined(node, resolvedRowAxis)) {
          maxWidth =
            node.layout[dim[resolvedRowAxis]] - paddingAndBorderAxisResolvedRow;
  ...
  if (linesCount > 1 && isCrossDimDefined) {
    ...
    var /*float*/ remainingAlignContentDim =
