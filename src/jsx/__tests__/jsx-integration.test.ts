...
import type { Element, SlideElement, TextElement, HeadingElement } from '../../types/elements';
...
describe('JSX Integration', () => {
  it('JSXからSlideWeave形式に正しく変換される', () => {
    ...
    const container = createSlideElement('container', {
  ...
  it('複雑なネスト構造が正しく処理される', () => {
    ...
    const leftCol = createSlideElement('container', { style: { flex: 1 } }, leftText);
    const rightCol = createSlideElement('container', { style: { flex: 1 } }, rightText);
  ...
  it('文字列以外のchildrenはcontentに含まれない', () => {
    ...
    const element = createSlideElement('heading', {}, 'Main heading', childElement);
