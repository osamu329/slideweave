...
describe("JSX Factory", () => {
  describe("createSlideElement", () => {
    ...
    it("null/undefined/false/空文字のchildrenをフィルタする", () => {
      ...
      expect(element).toEqual({
        type: "container",
        children: [{ type: "text", content: "Valid" }, 0],
      });
    });
    ...
    it("ネストした配列のchildrenを平坦化する", () => {
      ...
      const element = createSlideElement("container", {}, child1, [
        child2,
        child3,
      ]);
    ...
    it("複雑なレイアウト構造を作成する", () => {
      ...
      const leftColumn = createSlideElement(
        "container",
        {
          style: { flex: 1 },
        },
      ...
      const rightColumn = createSlideElement(
        "container",
        {
          style: { flex: 1 },
        },
  ...
  describe("Fragment", () => {
    ...
    it("単一の子要素を配列で返す", () => {
      ...
      expect(fragment).toEqual([{ type: "text", content: "Single Child" }]);
    });
