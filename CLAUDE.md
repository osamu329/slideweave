...
### 検証結果分析前の強制手順（例外なし実行）
- [ ] 検証プログラムの結果を数値・事実のみで要約（解釈なし）
- [ ] 「推測」「可能性」「おそらく」等の憶測語句使用禁止  
- [ ] CLAUDE.mdの関連仕様を引用・確認
- [ ] 検証結果と仕様の整合性確認
- [ ] 憶測による分析を行っていないか自己チェック
- [ ] 検証プログラムの出力を都合よく解釈していないか確認
- [ ] 浅い判定（数だけ見て詳細を無視）をしていないか確認

### 禁止事項
❌ **絶対禁止**: 検証結果を無視した推測・憶測による分析  
❌ **絶対禁止**: 検証プログラムの出力の都合の良い解釈  
❌ **絶対禁止**: 数値の表面的な判定（詳細データの無視）  
❌ **絶対禁止**: 仕様確認なしの独自判断  

### 違反防止ルール
- 検証・比較タスク開始時に必ずTodoWriteでチェックリスト追加
- 全項目完了後でないと分析結果を発言禁止
- 仕様不明時はRead toolでCLAUDE.md確認必須
- 検証結果に矛盾する発言は即座に停止・訂正

**目的**: 検証結果を無視した憶測分析・都合の良い解釈の完全防止
...
### 問題発生時の対応原則
- **禁止**: 一時的なテストファイルを大量作成する
- **必須**: 既存のテストファイルを上書きして比較する

### テストファイル作成時の強制手順
1. **既存ファイル確認**: 同じ目的のテストファイルが既にないか確認
2. **命名規則遵守**: 元のJSONファイル名と一致する出力ファイル名を使用
3. **一時ファイル禁止**: `-debug`, `-fixed`, `-temp`, `-test`等の接尾辞禁止
4. **即座削除**: 作業完了後に不要ファイルを即座削除
...
### ファイル命名規則
```
✅ 正しい: examples/test16-3-flex-flexbasis-deck.json → examples/output/test16-3-flex-flexbasis-deck.pptx
❌ 間違い: examples/test16-3-flex-flexbasis-deck.json → examples/output/test16-3-debug.pptx
❌ 間違い: examples/test16-3-flex-flexbasis-deck.json → examples/output/test16-3-fixed.pptx
```

### 違反防止ルール
- 3個以上のテストファイル作成時は即座に停止
- ユーザーから「テストケース増やすな」指摘時は全削除
- 比較対象が不明になった時点で作業リセット

## 🎯 問題解決の方向性判定ルール

### 修正方向性の判定手順
1. **現状把握**: どちらが期待される動作かを確認
2. **仕様確認**: ドキュメント・既存動作から正解を特定  
3. **良い方に統一**: 悪い動作を良い動作に合わせる
4. **実用性優先**: 技術的一貫性より実際の利用価値を重視

### 判定時の禁止事項
❌ **絶対禁止**: 既存コードがあるから正しいと決めつける
❌ **絶対禁止**: 一貫性のために悪い方に統一する  
❌ **絶対禁止**: 問題を隠蔽して解決済みとする
❌ **絶対禁止**: 検証せずに方向性を決める
